...
export default function LiveElectionPage() {
    ...
    async function getBallotsByCandidates(canndidateID: string) {
        const ballots = await getVotes(election!.id)
        return ballots.filter((ballot) => ballot.candidate === canndidateID).map((b) => b.id);
    }
    ...
    async function handleEndElection() {
        ...
        for (const candidate of candidates) { // Use for loop for clarity
          const ballots = await getBallotsByCandidates(candidate.id);
      
          await createCandidatesResults({
            ballot: ballots,
